...
class CentralizedActorRNN:
    ...
    def __init__(
        ...
        **agent_kwargs,
        ...
        if normalize_distances:
            self.normalize_distances = lambda x: np.clip(
                (x - (-max_range_dist)) / (max_range_dist - (-max_range_dist)),
                0.0,
                1.0,
            )
        else:
    ...
    def reset(self, seed: Optional[int] = None, pos = [0.,0.,0.]):
        ...
        if seed is not None:
            ...
            np.random.seed(seed)
        self.last_pos = {
            agent: torch.tensor([pos[0], pos[1], pos[2]], device=self.device)
    ...
    def step(
        ...
        with torch.no_grad():
            ...
            print('Processed Obs:',processed_obs)
